...
@section('title', 'Edit Discount - Nami Lontar')
    ...
    <form action="{{ route('admin.discounts.update', $discount->id) }}" method="POST">
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="text" id="code" name="code" placeholder="e.g. AUTUMN25" value="{{ old('code', $discount->code) }}" style="text-transform: uppercase;" required>
                @error('code')
            ...
            <div class="form-group">
                ...
                <select id="type" name="type" required>
                    ...
                    <option value="Shipping" {{ old('type', $discount->type) == 'Shipping' ? 'selected' : '' }}>Free Shipping</option>
                </select>
                @error('type')
                    <span style="color: var(--status-red-text); font-size: 0.85rem; margin-top: 5px; display: block;">{{ $message }}</span>
                @enderror
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="number" id="amount" name="amount" step="0.01" min="0" placeholder="e.g. 20" value="{{ old('amount', $discount->amount) }}" required>
                @error('amount')
                    <span style="color: var(--status-red-text); font-size: 0.85rem; margin-top: 5px; display: block;">{{ $message }}</span>
                @enderror
            </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <input type="date" id="start_date" name="start_date" value="{{ old('start_date', $discount->start_date) }}">
                @error('start_date')
                    <span style="color: var(--status-red-text); font-size: 0.85rem; margin-top: 5px; display: block;">{{ $message }}</span>
                @enderror
            </div>
            <div class="form-group">
                ...
                <input type="date" id="end_date" name="end_date" value="{{ old('end_date', $discount->end_date) }}">
                @error('end_date')
                    <span style="color: var(--status-red-text); font-size: 0.85rem; margin-top: 5px; display: block;">{{ $message }}</span>
                @enderror
            </div>
